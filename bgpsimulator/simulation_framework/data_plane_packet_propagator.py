...
class DataPlanePacketPropagator:
    def get_as_outcomes_for_data_plane_packet(self, dest_ip_addr: IPAddr, simulation_engine: SimulationEngine, legitimate_origin_asns: set[int], attacker_asns: set[int], scenario: Scenario | None = None) -> dict[int, Outcomes]:
        ...
        for as_obj in simulation_engine.as_graph:
            self.store_as_data_plane_outcomes(as_obj, simulation_engine.as_graph, dest_ip_addr, outcomes=outcomes, visited_asns=set(), legitimate_origin_asns=legitimate_origin_asns, attacker_asns=attacker_asns, scenario=scenario)
        return outcomes

    def store_as_data_plane_outcomes(self, as_obj: AS, engine: SimulationEngine, dest_ip_addr: IPAddr, outcomes: dict[int, Outcomes], visited_asns: set[int], legitimate_origin_asns: set[int], attacker_asns: set[int], scenario: Scenario | None = None):
        ...
        if as_obj.asn in outcomes:
            ...
            if outcome == Outcomes.UNDETERMINED:
                ...
                outcome = self.store_as_data_plane_outcomes(next_as, engine, dest_ip_addr, outcomes, visited_asns, legitimate_origin_asns, attacker_asns, scenario)
            outcomes[as_obj.asn] = outcome
            return outcome

    def _determine_data_plane_outcome(self, as_obj: AS, engine: SimulationEngine, dest_ip_addr: IPAddr, most_specific_ann: Ann, visited_asns: set[int], legitimate_origin_asns: set[int], attacker_asns: set[int], scenario: Scenario) -> Outcomes:
