...
class RIBsOut(UserDict[int, dict[Prefix, Ann]]):
    ...
    def to_json(self) -> dict[int, dict[str, dict[str, dict[str, Ann]]]]:
        ...
        return {
            neighbor_asn: {str(prefix): ann.to_json() for prefix, ann in prefix_anns.items()}
            for neighbor_asn, prefix_anns in self.data.items()
        }

    @classmethod
    def from_json(cls, json: dict[int, dict[str, dict[str, dict[str, Ann]]]]) -> "RIBsOut":
        """Returns a RIBsOut from a JSON representation"""

        ribs_out = cls()
        for neighbor_asn, prefix_anns in json.items():
            for prefix, ann_json in prefix_anns.items():
