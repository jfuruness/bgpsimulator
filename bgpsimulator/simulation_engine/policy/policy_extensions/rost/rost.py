...
class ROST:
    ...
    def add_suppressed_withdrawals_back_to_recv_q(policy: "Policy") -> None:
        for inner_dict in policy.ribs_in.values():
            for ann_info in inner_dict.values():
                ...
                for ann in policy.recv_q.get(ribs_in_ann.prefix, []):
                ...
                if (
                    ...
                    and policy.rost_trusted_repository.seen_withdrawal(ribs_in_ann)
                ):
                    policy.recv_q[ribs_in_ann.prefix].append(
                        ribs_in_ann.copy(withdraw=True)
                    )
