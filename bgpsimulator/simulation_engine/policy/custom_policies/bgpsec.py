...
class BGPSec:
    ...
    def bgpsec_valid(policy: "Policy", ann: "Ann") -> bool:
        ...
        return ann.bgpsec_next_asn == policy.as_.asn and ann.bgpsec_as_path == ann.as_path
    ...
    def get_policy_propagate_vals(policy: "Policy", neighbor_as: "AS", ann: "Ann", propagate_to: Relationships, send_rels: set[Relationships]) -> PolicyPropagateInfo:
        ...
        neighbor_bgpsec = any(neighbor_as.policy.settings.get(setting, False) for setting in [Settings.BGPSEC, Settings.BGP_I_SEC, Settings.BGP_I_SEC_TRANSITIVE])
        if neighbor_bgpsec:
        ...
        return PolicyPropagateInfo(policy_propagate_bool=True, ann=send_ann, send_ann_bool=True)
        
    @staticmethod
    def process_bgpsec_ann(policy: "Policy", ann: "Ann", from_rel: Relationships) -> "Ann":
    ...
    @staticmethod
    def get_best_ann_by_bgpsec(policy: "Policy", current_ann: "Ann", new_ann: "Ann") -> "Ann | None":
